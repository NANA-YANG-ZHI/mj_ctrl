"""
Collect NPZ experiment files into a summary CSV for the method comparison sweep.

Scans  <data_root>/<robot>/<method>/data_*.npz  for every combination,
trims burn-in, computes mean and std of force / position error,
and writes one row per (robot, method, speed) to the output CSV.

Usage:
    python collect_method_comparison.py <data_root>
        [--output results.csv]
        [--skip-seconds 1.0]
        [--dt 0.001]
"""
...
def main():
    parser = argparse.ArgumentParser(description="Collect method-comparison NPZ files → CSV")
    ...
    output = args.output or os.path.normpath(os.path.join(args.data_root, "..", "results.csv"))
