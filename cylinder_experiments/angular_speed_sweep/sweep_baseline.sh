#!/usr/bin/env bash
# Angular-speed sweep for the Baseline controller (run_baseline_mujoco.py).
#
# Sweeps omega from 0.1 to 1.6 rad/s (step 0.1) across four robot configs.
# Saves per-run NPZ data, then collects results into sweep_results.csv.
#
# Usage (from workspace root):
#   bash cylinder_experiments/angular_speed_sweep/sweep_baseline.sh
#
# Environment overrides:
#   NUM_WORKERS      parallel jobs              (default: 8)
#   SKIP_SECONDS     burn-in seconds for CSV    (default: 1.0)
...
OUTPUT_DIR="${SCRIPT_DIR}/sweep_speed_results/baseline"
...
run_one() {
    ...
    echo "[${job_id}] DONE   robot=${robot}  omega=${omega}  → ${data_dir}/data_${omega}.npz"
}
