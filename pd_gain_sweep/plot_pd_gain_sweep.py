"""Plot Kp x Kd heatmap of steady-state force error from PD gain sweep CSV.

Usage:
    python plot_pd_gain_sweep.py <results_csv> <output_dir>
"""
import sys
import os
import numpy as np
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python plot_pd_gain_sweep.py <results_csv> <output_dir>")
        sys.exit(1)
    ...
    kd_unique = sorted(set(kd_vals))
    nkp = len(kp_unique)
    nkd = len(kd_unique)

    kp_idx = {v: i for i, v in enumerate(kp_unique)}
    kd_idx = {v: i for i, v in enumerate(kd_unique)}

    def make_grid(values):
        grid = np.full((nkp, nkd), float("nan"))
        for kp, kd, v in zip(kp_vals, kd_vals, values):
            grid[kp_idx[kp], kd_idx[kd]] = v
        return grid
    ...
    fig.suptitle("PD Gain Sweep — Steady-State Performance", fontsize=14)
    ...
    KD = np.array(kd_unique)

    for ax, grid, title, cmap in specs:
        im = ax.pcolormesh(KD, KP, grid, cmap=cmap, shading="auto")
        ...
        if not np.all(np.isnan(grid)):
            ...
            best_kd = KD[min_idx[1]]
            ax.plot(best_kd, best_kp, "w*", markersize=14,
                    label=f"min: Kp={best_kp}, Kd={best_kd}")
