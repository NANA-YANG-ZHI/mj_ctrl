...
### version 1
- Implementation and evaluation of the Hybrid Force-Impedance Control framework (Iskandar et al., 2023) in simulation and on real hardware
    - Systematic study of force control methods (F_desired, PD, PI, velocity-term compensation) on flat and slope surfaces
    - Extension of the controller from flat (φ = z) to inclined surface (slope 30°) using constraint-frame Jacobians
    - Transfer and validation on the Franka Panda real robot using libfranka and Pinocchio
- Analysis of friction effects on contact force regulation
- learning-based ppo: compare ppo based compensation and paper method in simulation/ real robot. (motivation: paper method using separate space control, but performs not so good on surface with friction, so I use ppo and output a compensation torque and try to address the force errors in env with friction)
...
*Describe the MuJoCo simulation environment: Franka FR3. Scene construction via XML with procedurally added slope geometry (`add_slope_xml`). Contact model settings (condim, friction). Simulation timestep (1 ms) same as libfranka.*

### 3.1.2 Real-Robot Setup (Franka FR3 + libfranka)

*Describe the real-robot hardware interface: Franka Research 3 (FR3) via libfranka. Torque control at 1 kHz. Pinocchio used for dynamics (Jacobian, inertia matrix, Coriolis, gravity) in both sim and real.*
...
*Trajectory parametrisation: angular speed swept from π*0,1 to π*5 rad/s (EE linear speed 0.0314–1.57 m/s at radius 0.1 m), circle radius = 0.1 m. Circle centre position and target orientation.*
...
### 3.4.3 PI Force Controller - TODO

✏️ *Add integral term to eliminate steady-state error. Kp, Ki tuning discussion.*
### 3.4.4 Hybrid Force-Impedance with Velocity-Term Compensation (Paper Method)
...
## 4.5 Effect of Selection Matrix Dimensionality

✏️ *Compare S_f as 3D (earlier approach) vs. 1D (z-only normal force). Discuss the quaternion/rotation issue when S_v omits rx/ry — head tilt artifact.*

📊 `PLOT` Side-by-side: EE orientation error with 3D vs. 1D S_f selection matrix

📊 `PLOT` X-plane and Z-plane view of EE trajectory showing head tilt problem

## 4.6 Effect of Friction
...
## 4.7 Impedance Gain Sensitivity
