#!/usr/bin/env bash
# 2-D grid search over PI gains (Kp_force x Ki_force).
# Runs run_approach_then_hybrid_mujoco.py headless for every (Kp, Ki) pair,
...
REPO_DIR="${SCRIPT_DIR}/.."
PLOT_SCRIPT="${SCRIPT_DIR}/plot_pi_gain_sweep.py"

# ---------------------------------------------------------------
# Configure the sweep here
# Results saved to: pi_gain_sweep/plots/<SWEEP_NAME>/
#
...
# KP_VALUES / KI_VALUES: space-separated lists of gain values to sweep
# NUM_WORKERS: parallel simulations
# ---------------------------------------------------------------
SWEEP_NAME="pi_gain_sweep"
FORCE_CONTROL_METHOD="paper"
ANGULAR_SPEED="6.283185307179586"   # pi*2 rad/s
SKIP_SECONDS="1.0"

KP_VALUES="0.1 0.5 1.0 2.0 3.0 5.0"
KI_VALUES="0.1 0.5 1.0 2.0 3.0 5.0"
NUM_WORKERS=10
...
# ---------------------------------------------------------------
# Worker function: runs one (Kp, Ki) pair, writes result to a temp file.
# ---------------------------------------------------------------
run_one() {
    local idx=$1
    local KP=$2
    ...
    echo "${KP},${KI},${AVG_FORCE_ERROR},${VAR_FORCE_ERROR},${AVG_POS_ERROR},${VAR_POS_ERROR}" \
        > "${TMP_DIR}/result_${idx}.csv"

    echo "[worker ${idx}] done — Kp=${KP} Ki=${KI}  avg_force_error=${AVG_FORCE_ERROR}  var=${VAR_FORCE_ERROR}"
}
...
# ---------------------------------------------------------------
# Build full list of (Kp, Ki) pairs and dispatch workers
# ---------------------------------------------------------------
KP_ARR=($KP_VALUES)
KI_ARR=($KI_VALUES)

total=$(( ${#KP_ARR[@]} * ${#KI_ARR[@]} ))
echo "Starting PI gain sweep: ${#KP_ARR[@]} Kp values × ${#KI_ARR[@]} Ki values = ${total} runs  (NUM_WORKERS=${NUM_WORKERS})"
...
# ---------------------------------------------------------------
echo "kp,ki,avg_force_z_error,var_force_z_error,avg_position_error,var_position_error" > "${RESULTS_CSV}"
